...
Object.assign(soundBtn.style,{
    ...
    width:'50px', height:'50px', cursor:'pointer', zIndex:1000
});
...
function spawnObstacle(){
    ...
    let img = obstacleImgs[Math.floor(Math.random() * obstacleImgs.length)];
    let height = baseHeight + Math.random()*extraHeight;
    let speed = 8 + Math.floor(score/10);
    let spawnRate = Math.max(110,160-score*2);
...
function spawnCoin(){
    ...
    let speed = 8 + Math.floor(score/10);
    coins.push({x:canvas.width,y:y,width:40,height:40,speed:speed});
}
...
// وضعیت بازی
// ===========================
let frame=0, score=0, gameOver=false, nextSpawn=0, currentWave=0, waveTextTimer=2;
...
document.addEventListener('keydown', e=>{if(e.code==='Space'||e.code==='ArrowUp') jumpAction();});

const jumpBtn=document.createElement('button');
jumpBtn.textContent="JUMP";
Object.assign(jumpBtn.style,{
    position:'absolute',bottom:'30px',right:'30px',
    width:'100px',height:'100px',fontSize:'20px',
...
function resetGame(){
    ...
    obstacles = [];
    coins = [];
    frame=0; score=0; gameOver=false; nextSpawn=0;
    overPlayed=false;
    update();
}
...
function update(){
    if(gameOver){
        ...
        if(!overPlayed){overSound.currentTime=0; overSound.play(); overPlayed=true;}
        return;
    }
    ...
    if(newWave!==currentWave){currentWave=newWave; waveTextTimer=120;}
    ...
    // ایجاد مانع
    if(frame>=nextSpawn) nextSpawn=frame+spawnObstacle();
    ...
    if(waveTextTimer>0){
        ...
        ctx.font='40px Arial';
        ctx.textAlign='center';
        ctx.fillText(`Wave ${currentWave+1}`,canvas.width/2,80);
        waveTextTimer--;
    }
